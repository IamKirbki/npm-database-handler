...
describe('FromParser', () => {
    ...
    describe('ParseTables - Table Aliases', () => {
        it('should parse table with AS alias', () => {
            ...
            expect(parser.ParseTables()).toEqual([{ tableName: 'users', alias: 'u' }]);
        });
        ...
        it('should parse schema-qualified table with alias', () => {
            ...
            expect(parser.ParseTables()).toEqual([{ tableName: 'main.users', alias: 'u' }]);
        });

        it('should parse multiple tables with aliases', () => {
            const parser = new FromParser('SELECT * FROM users AS u, orders AS o');
            expect(parser.ParseTables()).toEqual([{ tableName: 'users', alias: 'u' }, { tableName: 'orders', alias: 'o' }]);
        });

        it('should parse mixed aliased and non-aliased tables', () => {
            const parser = new FromParser('SELECT * FROM users AS u, orders, products p');
            expect(parser.ParseTables()).toEqual([{ tableName: 'users', alias: 'u' }, { tableName: 'orders' }, { tableName: 'products', alias: 'p' }]);
        });
    });
    ...
    describe('ParseTables - Subqueries', () => {
        it('should parse subquery in FROM clause', () => {
            ...
            expect(parser.ParseTables()).toBeDefined();
        });
    ...
    describe('ParseTables - WITH Clauses and Other Statements', () => {
        it('should parse UPDATE statement', () => {
            const parser = new FromParser('UPDATE users SET name = @name WHERE id = @id');
            expect(() => parser.ParseTables()).toThrow('Invalid SQL query: FROM clause not found.');
        });

        it('should parse DELETE FROM statement', () => {
            const parser = new FromParser('DELETE FROM users WHERE id = @id');
            expect(parser.ParseTables()).toEqual([{ tableName: 'users' }]);
        });

        it('should parse INSERT INTO statement', () => {
            const parser = new FromParser('INSERT INTO users (name, email) VALUES (@name, @email)');
            expect(() => parser.ParseTables()).toThrow('Invalid SQL query: FROM clause not found.');
        });
    });
    ...
    describe('ParseTables - WITH WHERE Clause', () => {
        ...
        it('should parse multiple tables with WHERE clause', () => {
            ...
            expect(parser.ParseTables()).toEqual([{ tableName: 'users' }, { tableName: 'orders' }]);
        });
    });
    ...
    describe('ParseTables - Edge Cases', () => {
        ...
        it('should handle table names with underscores', () => {
            ...
            expect(parser.ParseTables()).toEqual([{ tableName: 'user_accounts' }]);
        });

        it('should handle table names with numbers', () => {
            const parser = new FromParser('SELECT * FROM table1, table2');
            expect(parser.ParseTables()).toEqual([{ tableName: 'table1' }, { tableName: 'table2' }]);
        });
